...
const uploadAudio = async (req, res) => {
  try {
    ...
    if (!checkMp3(audioFile.name)) {
      return res
        .status(400)
        .json({ status: false, message: "Only mp3 audio is allowed." });
    }
    ...
    if (uploadedAudioFiles.includes(audioFile.name)) {
      return res
        .status(400)
        .json({ status: false, message: "Audio file name already exists." });
    }
...
const getAudios = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ status: true, message: "success", data: uploadedAudioFiles });
  } catch (error) {
    return res
      .status(500)
      .json({ status: false, message: "something went wrong", error });
...
const deleteAudio = async (req, res) => {
  try {
    ...
    if (!fs.readdirSync(`${APP_ROOT_PATH}/assets/audio/`).includes(audioFile)) {
      return res
        .status(400)
        .json({ status: false, message: "File doesn't exist" });
    }
...
const createCall = async (req, res) => {
  const { To, From, Url, Method } = req.body;
  try {
    ...
    return res
      .status(200)
      .json({ status: true, message: `call created to number:: ${To}`, data });
  } catch (err) {
    console.log("create call error:", err.response.data);
    return res
      .status(500)
      .json({ status: false, message: `call failed`, ...err.response.data });
  }
};
